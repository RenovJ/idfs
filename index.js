...
const osbConfig = {
  ...
  httpEndpoint: `${config.CHAIN_PROTOCOL}://${config.CHAIN_HOST}:${config.CHAIN_PORT}`,
  broadcast: true,
  verbose: false,
  sign: true,
...
app.post('/v0/add_data', async function(req, res){
	...
	if (isDataEncrypted === true) {
		...
		const decryptKeyBuffer = await decode(encryptedDecryptKeyBuffer, config.IDFS_PRIVATE_KEY)
...
app.get('/v0/get_public_key', function(req, res){
	res.json({
		result: true,
		public_key: EosjsEcc.privateToPublic(config.IDFS_PRIVATE_KEY, config.PUBKEY_PREFIX)
	})
})
